...
object SquareDeposit {
  ...
  implicit val squareDepositEncoder: RowEncoder[SquareDeposit] = RowEncoder.ordered {
   ...
   */
  def coalesceDepositDetails(depositDetails: Iterable[SquareDeposit]): Iterable[SquareDeposit] =
    depositDetails.groupBy(x => (x.date, x.depositId)).map { case ((date, depositId), squareDeposits) =>
